...
def _evaluate_nominal_and_moved(
    ...
    shwfs = real_shwfs._build_shwfs_measurement_model(nominal_model, focus_shift_mm=bench.REFERENCE_FOCUS_MM)
...
def run_repair(config: RunConfig, log: LogCallback | None = None) -> dict[str, Any]:
    ...
    bounds = refined_nominal._delta_bounds(
        ...
        ACTUATOR_IDS,
        base_state,
        local_radius_mm=bench.LOCAL_RADIUS_MM,
    )
    nominal_seed = bench._nominal_reset_seed(ACTUATOR_IDS, base_state, bounds)
    ...
    def objective(delta_vector: np.ndarray) -> float:
        ...
        t0 = time.perf_counter()
        psf_image = moved_model.get_psf_image(focus=bench.REFERENCE_FOCUS_MM)
        rmse = float(
            np.sqrt(
                np.mean(
    ...
    try:
        ...
        local_log(
            ...
            f"local_radius_mm={'full_mechanical' if bench.LOCAL_RADIUS_MM is None else f'{bench.LOCAL_RADIUS_MM:.6f}'} "
        ...
        with warnings.catch_warnings(record=True) as warning_records:
            ...
            try:
                minimize(
                    ...
                    bounds=bounds,
                    options={
                        "maxiter": bench.OPT_MAXITER,
                        "maxfun": bench.OPT_MAXFUN,
                        "ftol": 1.0e-6,
        ...
        summary = {
            "status": "complete",
            "algorithm": "adaptive_optics_v2_repair",
            "configuration": {
                "wavelength_nm": float(bench.WAVELENGTH_NM),
                "object_space_mm": float(bench.OBJECT_SPACE_MM),
                "reference_focus_mm": float(bench.REFERENCE_FOCUS_MM),
