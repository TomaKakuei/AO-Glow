...
TT_DOMINANT_PRIMARY_RATIO = float(os.environ.get("AO_V9_TT_DOMINANT_PRIMARY_RATIO", "0.85"))
...
class RunConfig:
    ...
    shwfs_lenslet_count: int | None = None
    shwfs_slope_limit: int | None = None
    shwfs_canonical_active_lenslets: int | None = None
...
class SensorPreset:
    lenslet_count: int
    slope_limit: int
    canonical_active_lenslets: int


PROFILE_SENSOR_PRESETS: dict[str, SensorPreset] = {
    "realistic": SensorPreset(lenslet_count=13, slope_limit=256, canonical_active_lenslets=128),
    "minimal": SensorPreset(lenslet_count=17, slope_limit=512, canonical_active_lenslets=160),
    "hard": SensorPreset(lenslet_count=13, slope_limit=256, canonical_active_lenslets=128),
    "none": SensorPreset(lenslet_count=13, slope_limit=256, canonical_active_lenslets=128),
}
...
class RelativeRepairPlant:
    ...
    def evaluate_relative_delta(
        ...
        *,
        ...
        current_slopes, current_coeffs = shwfs.estimate_from_model(self.optical_model)
        slope_residual = np.asarray(current_slopes, dtype=np.float64) - np.asarray(shwfs.reference_slopes, dtype=np.float64)
        estimated_mechanical_delta = None
        if getattr(shwfs, "mechanical_response_pinv", np.empty((0, 0))).size > 0:
            estimated_mechanical_delta = np.asarray(
                shwfs.estimate_mechanical_delta(slope_residual),
                dtype=np.float64,
            )
        return {
            ...
            "estimated_mechanical_delta": estimated_mechanical_delta,
        }
...
def _configure_v5_shwfs_runtime(config: RunConfig) -> str:
    ...
    preset = PROFILE_SENSOR_PRESETS.get(
        profile_name,
        SensorPreset(
            lenslet_count=int(DEFAULT_SHWFS_LENSLET_COUNT),
            slope_limit=int(DEFAULT_SHWFS_SLOPE_LIMIT),
            canonical_active_lenslets=128,
        ),
    )
    real_shwfs.configure_shwfs_runtime(
        lenslet_count=int(config.shwfs_lenslet_count or preset.lenslet_count),
        slope_limit=int(config.shwfs_slope_limit or preset.slope_limit),
        ...
        canonical_active_lenslets=int(
            config.shwfs_canonical_active_lenslets or preset.canonical_active_lenslets
        ),
    )
    return profile_name
...
def _residual_block_signature(
    residual_vector: np.ndarray,
    *,
    coeff_noise_std: np.ndarray | None = None,
    estimated_mechanical_delta: np.ndarray | None = None,
    block_indices: dict[str, np.ndarray] | None = None,
    search_box_xy_radius_mm: float | None = None,
    search_box_z_radius_mm: float | None = None,
    tt_limit_deg: float | None = None,
) -> dict[str, float]:
    ...
    if residual.size <= 0:
        return {
            ...
            "lateral_over_radial": 0.0,
            "angular_over_primary": 0.0,
            "higher_over_angular": 0.0,
        }

    whitened = residual.copy()
    if coeff_noise_std is not None:
        scale = np.asarray(coeff_noise_std, dtype=np.float64).reshape(-1)
        if scale.size == residual.size:
            whitened = residual / np.maximum(scale, 1.0e-12)
    ...
    def abs_mode(pair: tuple[int, int]) -> float:
        ...
        if idx is None:
            return 0.0
        return float(abs(whitened[idx]))

    def group_rms(pairs: tuple[tuple[int, int], ...]) -> float:
        values: list[float] = []
        for pair in pairs:
            idx = pair_to_index.get(pair)
            if idx is None:
                continue
            values.append(float(whitened[idx]))
        if not values:
            return 0.0
    ...
    if estimated_mechanical_delta is not None and block_indices is not None:
        mechanical = np.asarray(estimated_mechanical_delta, dtype=np.float64).reshape(-1)
        xy_scale = max(float(search_box_xy_radius_mm or 0.0), 1.0e-12)
        z_scale = max(float(search_box_z_radius_mm or 0.0), 1.0e-12)
        tt_scale = max(float(tt_limit_deg or 0.0), 1.0e-12)

        def rms_norm(indices: np.ndarray, scale_value: float) -> float:
            if indices.size <= 0:
                return 0.0
            values = np.asarray(mechanical[indices], dtype=np.float64).reshape(-1)
            return float(np.sqrt(np.mean(values * values, dtype=np.float64)) / max(scale_value, 1.0e-12))

        lateral = rms_norm(block_indices["xy"], xy_scale)
        radial = rms_norm(block_indices["z"], z_scale)
        angular_u = rms_norm(block_indices["tt_x"], tt_scale)
        angular_v = rms_norm(block_indices["tt_y"], tt_scale)
        angular = float(np.hypot(angular_u, angular_v))

    if radial <= 0.0 and lateral <= 0.0 and angular <= 0.0:
        radial = group_rms(((0, 0), (2, 0)))
        lateral = abs_mode((1, -1)) + abs_mode((1, 1))
        if radial <= 0.0:
            radial = float(np.sum(np.abs(whitened)))
        angular_u = group_rms(((1, -1), (3, -1)))
        angular_v = group_rms(((1, 1), (3, 1)))
        angular = float(np.hypot(angular_u, angular_v))
    ...
    for idx, pair in enumerate(mode_pairs):
        if pair in {(0, 0), (2, 0), (1, -1), (3, -1), (1, 1), (3, 1)}:
            continue
        higher += float(whitened[idx] * whitened[idx])
    if higher > 0.0:
        higher = float(np.sqrt(higher / max(1, int(residual.size))))
    primary = max(radial, lateral, 1.0e-12)
    return {
        "radial": radial,
        "lateral": lateral,
        "angular_u": angular_u,
        ...
        "lateral_over_radial": float(lateral / max(radial, 1.0e-12)),
        "angular_over_primary": float(angular / primary),
...
def _choose_primary_block_order(signature: dict[str, float]) -> list[str]:
    if float(signature["lateral"]) <= float(signature["radial"]):
        return ["z", "xy"]
    return ["xy", "z"]


def _tt_dominant_case(signature: dict[str, float]) -> bool:
    return bool(ENABLE_TIPTILT and float(signature["angular_over_primary"]) >= float(TT_DOMINANT_PRIMARY_RATIO))
...
def run_repair(config: RunConfig, log: LogCallback | None = None) -> dict[str, Any]:
    ...
    def inspect_signature(delta_vector: np.ndarray) -> dict[str, Any]:
        ...
        signature = _residual_block_signature(
            np.asarray(result["residual"], dtype=np.float64),
            coeff_noise_std=getattr(shwfs, "coeff_noise_std", None),
            estimated_mechanical_delta=result.get("estimated_mechanical_delta"),
            block_indices=block_indices,
            search_box_xy_radius_mm=search_box_xy_radius_mm,
            search_box_z_radius_mm=search_box_z_radius_mm,
            tt_limit_deg=float(TIPTILT_TEST_LIMIT_ARCMIN / 60.0),
        )
    ...
    def optimize_block(
        ...
        *,
        ...
        local_log(
            ...
            f"xy_over_z={stage_signature_before['lateral_over_radial']:.3f} "
            f"tt_over_primary={stage_signature_before['angular_over_primary']:.3f} "
        ...
        with warnings.catch_warnings(record=True) as warning_records:
            ...
            try:
                ...
                local_log(
                    ...
                    f"initial xy_over_z={initial_signature['lateral_over_radial']:.3f} "
                    f"tt_over_primary={initial_signature['angular_over_primary']:.3f} "
