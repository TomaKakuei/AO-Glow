...
from w_bessel_core import generate_w_bessel_basis, project_phase_map, reconstruct_phase_map
...
LENSLET_COUNT = _env_int("AO_V6_SHWFS_LENSLET_COUNT", _env_int("AO_V5_SHWFS_LENSLET_COUNT", _env_int("AO_V3_SHWFS_LENSLET_COUNT", 13)))
SHWFS_MAXITER = 8
SHWFS_MAXFUN = 120
SHWFS_RCOND = _env_float("AO_V6_SHWFS_RCOND", 1.0e-3)
SHWFS_SLOPE_LIMIT = _env_int("AO_V6_SHWFS_SLOPE_LIMIT", _env_int("AO_V5_SHWFS_SLOPE_LIMIT", _env_int("AO_V3_SHWFS_SLOPE_LIMIT", 256)))
MECHANICAL_RESPONSE_STEP_MM = 5.0e-3
MECHANICAL_RESPONSE_STEP_SCHEDULE_MM = (5.0e-3,)
MECHANICAL_RESPONSE_RCOND = _env_float("AO_V6_MECHANICAL_RESPONSE_RCOND", 1.0e-6)
SHWFS_NOISE_SEED = _env_int("AO_V6_SHWFS_NOISE_SEED", _env_int("AO_V5_SHWFS_NOISE_SEED", _env_int("AO_V3_SHWFS_NOISE_SEED", 20260430)))
SHWFS_NOISE_PROFILE = _env_str("AO_V6_SHWFS_NOISE_PROFILE", _env_str("AO_V5_SHWFS_NOISE_PROFILE", "realistic")).strip().lower()
SHWFS_LENSLET_PIXEL_COUNT = _env_int("AO_V6_SHWFS_LENSLET_PIXEL_COUNT", _env_int("AO_V5_SHWFS_LENSLET_PIXEL_COUNT", 12))
SHWFS_FORWARD_AVERAGES = _env_int("AO_V6_SHWFS_FORWARD_AVERAGES", _env_int("AO_V5_SHWFS_FORWARD_AVERAGES", 1))
SHWFS_ENABLE_MEDIAN_FILTER = _env_int("AO_V6_SHWFS_ENABLE_MEDIAN_FILTER", 1) != 0
SHWFS_ENABLE_GAUSSIAN_FILTER = _env_int("AO_V6_SHWFS_ENABLE_GAUSSIAN_FILTER", 1) != 0
SHWFS_GAUSSIAN_SIGMA_PX = _env_float("AO_V6_SHWFS_GAUSSIAN_SIGMA_PX", 0.7)
SHWFS_DYNAMIC_THRESHOLD_SIGMA = _env_float("AO_V6_SHWFS_DYNAMIC_THRESHOLD_SIGMA", 3.0)
SHWFS_WEIGHTED_CENTROID_POWER = _env_float("AO_V6_SHWFS_WEIGHTED_CENTROID_POWER", 2.0)
SHWFS_WINDOW_RADIUS_PX = _env_int("AO_V6_SHWFS_WINDOW_RADIUS_PX", 5)
SHWFS_TIKHONOV_LAMBDA = _env_float("AO_V6_SHWFS_TIKHONOV_LAMBDA", 0.015)
SHWFS_ACTIVE_MODES = _env_int("AO_V6_SHWFS_ACTIVE_MODES", 6)
...
class ShwfsNoiseProfile:
    name: str
    description: str
    target_adu_fraction: float
    gain_std: float
    offset_mean_e: float
    offset_std_e: float
    read_noise_rms_e: float
    full_well_e: float = 10000.0
    conversion_gain_e_per_adu: float = 35.0
    adu_max: int = 255
...
NOISE_PROFILES: dict[str, ShwfsNoiseProfile] = {
    "none": ShwfsNoiseProfile(
        ...
        target_adu_fraction=0.8,
        gain_std=0.0,
        offset_mean_e=0.0,
        offset_std_e=0.0,
        read_noise_rms_e=0.0,
    ),
    "minimum": ShwfsNoiseProfile(
        name="minimum",
        description="High-quality operating point with strong exposure and light fixed-pattern noise.",
        target_adu_fraction=0.8,
        gain_std=0.01,
        offset_mean_e=20.0,
        offset_std_e=2.0,
        read_noise_rms_e=3.0,
    ),
    "realistic": ShwfsNoiseProfile(
        name="realistic",
        description="Typical closed-loop operating point with moderate exposure and realistic CMOS noise.",
        target_adu_fraction=0.5,
        gain_std=0.02,
        offset_mean_e=60.0,
        offset_std_e=6.0,
        read_noise_rms_e=5.0,
    ),
    "enhanced": ShwfsNoiseProfile(
        name="enhanced",
        description="Underexposed stress-test regime with stronger background and read noise.",
        target_adu_fraction=0.25,
        gain_std=0.04,
        offset_mean_e=150.0,
        offset_std_e=15.0,
        read_noise_rms_e=10.0,
    ),
}
...
def configure_shwfs_runtime(
    *,
    ...
    global LENSLET_COUNT, SHWFS_SLOPE_LIMIT, SHWFS_NOISE_PROFILE, SHWFS_NOISE_SEED, SHWFS_LENSLET_PIXEL_COUNT, SHWFS_FORWARD_AVERAGES
...
class ShwfsMeasurementModel:
    ...
    active_cell_linear_indices: tuple[int, ...] = ()
    ...
    enable_median_filter: bool = SHWFS_ENABLE_MEDIAN_FILTER
    enable_gaussian_filter: bool = SHWFS_ENABLE_GAUSSIAN_FILTER
    gaussian_sigma_px: float = SHWFS_GAUSSIAN_SIGMA_PX
    dynamic_threshold_sigma: float = SHWFS_DYNAMIC_THRESHOLD_SIGMA
    weighted_centroid_power: float = SHWFS_WEIGHTED_CENTROID_POWER
    window_radius_px: int = SHWFS_WINDOW_RADIUS_PX
    tikhonov_lambda: float = SHWFS_TIKHONOV_LAMBDA
    active_mode_count: int = SHWFS_ACTIVE_MODES
    ...
    def __post_init__(self) -> None:
        ...
        self.window_radius_px = max(1, int(self.window_radius_px))
        self.weighted_centroid_power = max(1.0, float(self.weighted_centroid_power))
        self.dynamic_threshold_sigma = max(0.0, float(self.dynamic_threshold_sigma))
        self.gaussian_sigma_px = max(0.0, float(self.gaussian_sigma_px))
        self.tikhonov_lambda = max(0.0, float(self.tikhonov_lambda))
        pixel_count = max(6, int(self.lenslet_pixel_count))
        self.active_mode_count = min(max(1, int(self.active_mode_count)), int(self.basis_matrix.shape[0]))
        ...
        for _ in range(len(self.active_cell_indices)):
            if self.noise_profile.gain_std > 0.0:
                gain_map = 1.0 + self.noise_profile.gain_std * fixed_rng.standard_normal(
                    (pixel_count, pixel_count)
                )
                gain_map = np.clip(gain_map, 0.5, 1.5)
            else:
                gain_map = np.ones((pixel_count, pixel_count), dtype=np.float64)
            if self.noise_profile.offset_std_e > 0.0 or self.noise_profile.offset_mean_e > 0.0:
                offset_map = self.noise_profile.offset_mean_e + self.noise_profile.offset_std_e * fixed_rng.standard_normal(
                    (pixel_count, pixel_count)
                )
    ...
    def measure_slopes(self, phase_waves: np.ndarray) -> np.ndarray:
        slopes, _ = self.measure_slopes_with_sensor_image(phase_waves)
        return slopes

    def measure_slopes_with_sensor_image(self, phase_waves: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        phase = np.asarray(phase_waves, dtype=np.float64)
        measurements: list[float] = []
        sensor_image = np.zeros(
            (
                int(self.lenslet_count) * int(self.lenslet_pixel_count),
                int(self.lenslet_count) * int(self.lenslet_pixel_count),
            ),
            dtype=np.float64,
        )
        ...
        for lenslet_index, flat_indices in enumerate(self.active_cell_indices):
            ...
            patch, slope_xy = self._measure_lenslet_patch_and_centroid_slopes(
                np.asarray(coeffs[:2], dtype=np.float64),
                lenslet_index,
            )
            self._insert_lenslet_patch(sensor_image, patch, lenslet_index)
            measurements.extend([float(slope_xy[0]), float(slope_xy[1])])
        return np.asarray(measurements, dtype=np.float64), sensor_image
    ...
    def measure_model_slopes_with_sensor_image(
        self,
        optical_model: RayOpticsPhysicsEngine,
    ) -> tuple[np.ndarray, np.ndarray]:
        phase = self._current_phase_waves(optical_model)
        return self.measure_slopes_with_sensor_image(phase)

    def estimate_coeffs(self, optical_model: RayOpticsPhysicsEngine) -> np.ndarray:
        slopes = self.measure_model_slopes(optical_model)
        return self._solve_modal_coeffs(slopes)

    def estimate_coeffs_with_sensor_image(
        self,
        optical_model: RayOpticsPhysicsEngine,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        slopes, sensor_image = self.measure_model_slopes_with_sensor_image(optical_model)
        coeffs = self._solve_modal_coeffs(slopes)
        return coeffs, slopes, sensor_image
    ...
    def _measure_lenslet_centroid_slopes(self, ideal_slope_xy: np.ndarray, lenslet_index: int) -> np.ndarray:
        _, slope_xy = self._measure_lenslet_patch_and_centroid_slopes(ideal_slope_xy, lenslet_index)
        return slope_xy

    def _measure_lenslet_patch_and_centroid_slopes(
        self,
        ideal_slope_xy: np.ndarray,
        lenslet_index: int,
    ) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self.noise_profile is None or self.noise_profile.name == "none":
            patch = self._ideal_lenslet_patch(slope_xy)
            return patch, slope_xy
        ...
        ideal_spot_normalized = self._ideal_lenslet_patch(slope_xy)
        target_peak_e = (
            float(self.noise_profile.target_adu_fraction)
            * float(self.noise_profile.adu_max)
            * float(self.noise_profile.conversion_gain_e_per_adu)
        )
        ideal_electrons = ideal_spot_normalized * target_peak_e
        ...
        sensor_adu = np.clip(
            np.rint(adc_input_electrons / max(float(self.noise_profile.conversion_gain_e_per_adu), 1.0e-12)),
            0.0,
            float(self.noise_profile.adu_max),
        )
        sensor_adu = np.asarray(sensor_adu, dtype=np.float64)
        processed_patch = self._preprocess_lenslet_patch(
            sensor_adu,
            expected_center_xy=np.asarray([center_px + shift_xy_px[0], center_px + shift_xy_px[1]], dtype=np.float64),
        )
        if not np.any(processed_patch > 0.0):
            return sensor_adu, np.zeros(2, dtype=np.float64)
        windowed_patch = self._window_patch_around_peak(processed_patch)
        weighted_patch = np.power(windowed_patch, float(self.weighted_centroid_power), dtype=np.float64)
        total_signal = float(np.sum(weighted_patch, dtype=np.float64))
        if total_signal <= 1.0e-12:
            return sensor_adu, np.zeros(2, dtype=np.float64)
        centroid_x = float(np.sum(self.pixel_x_grid * weighted_patch, dtype=np.float64) / total_signal)
        centroid_y = float(np.sum(self.pixel_y_grid * weighted_patch, dtype=np.float64) / total_signal)
        measured_shift_xy = np.asarray([centroid_x - center_px, centroid_y - center_px], dtype=np.float64)
        return sensor_adu, measured_shift_xy / max(float(self.noise_profile.slope_to_pixel_gain), 1.0e-12)

    def _ideal_lenslet_patch(self, slope_xy: np.ndarray) -> np.ndarray:
        if self.noise_profile is None:
            spot_sigma_px = 1.35
            slope_to_pixel_gain = 2.4
            max_centroid_shift_px = 3.0
        else:
            spot_sigma_px = float(self.noise_profile.spot_sigma_px)
            slope_to_pixel_gain = float(self.noise_profile.slope_to_pixel_gain)
            max_centroid_shift_px = float(self.noise_profile.max_centroid_shift_px)
        center_px = 0.5 * float(self.lenslet_pixel_count - 1)
        shift_xy_px = np.clip(
            np.asarray(slope_xy, dtype=np.float64).reshape(2) * slope_to_pixel_gain,
        ...
        return np.exp(-radius_sq / max(2.0 * spot_sigma_px * spot_sigma_px, 1.0e-12))

    def _insert_lenslet_patch(self, sensor_image: np.ndarray, patch: np.ndarray, lenslet_index: int) -> None:
        if not self.active_cell_linear_indices:
            return
        linear_index = int(self.active_cell_linear_indices[int(lenslet_index)])
        row = linear_index // int(self.lenslet_count)
        col = linear_index % int(self.lenslet_count)
        pixel_count = int(self.lenslet_pixel_count)
        row_lo = row * pixel_count
        col_lo = col * pixel_count
        sensor_image[row_lo : row_lo + pixel_count, col_lo : col_lo + pixel_count] = np.asarray(
            patch,
            dtype=np.float64,
        )

    def _preprocess_lenslet_patch(self, sensor_adu: np.ndarray, *, expected_center_xy: np.ndarray) -> np.ndarray:
        patch = np.asarray(sensor_adu, dtype=np.float64)
        if self.enable_median_filter:
            patch = median_filter(patch, size=3, mode="nearest")
        if self.enable_gaussian_filter and self.gaussian_sigma_px > 0.0:
            patch = gaussian_filter(patch, sigma=float(self.gaussian_sigma_px), mode="nearest", truncate=1.5)
        ...
        if background_pixels.size <= 0:
            ...
            background_level = float(np.median(background_pixels))
            mad = float(np.median(np.abs(background_pixels - background_level)))
            noise_sigma = 1.4826 * mad
        read_noise_adu = 0.0
        if self.noise_profile is not None:
            read_noise_adu = float(self.noise_profile.read_noise_rms_e) / max(
                float(self.noise_profile.conversion_gain_e_per_adu),
                1.0e-12,
            )
        threshold = background_level + float(self.dynamic_threshold_sigma) * max(noise_sigma, read_noise_adu)
        patch = np.where(patch >= threshold, patch - background_level, 0.0)
        patch = np.clip(patch, 0.0, None)
        return patch

    def _window_patch_around_peak(self, patch: np.ndarray) -> np.ndarray:
        working = np.asarray(patch, dtype=np.float64)
        if not np.any(working > 0.0):
            return working
        peak_flat = int(np.argmax(working))
        peak_row, peak_col = np.unravel_index(peak_flat, working.shape)
        radius = int(self.window_radius_px)
        row_lo = max(0, peak_row - radius)
        row_hi = min(working.shape[0], peak_row + radius + 1)
        col_lo = max(0, peak_col - radius)
        col_hi = min(working.shape[1], peak_col + radius + 1)
        mask = np.zeros_like(working, dtype=bool)
        mask[row_lo:row_hi, col_lo:col_hi] = True
        return np.where(mask, working, 0.0)

    def _solve_modal_coeffs(self, slopes: np.ndarray) -> np.ndarray:
        measurements = np.asarray(slopes, dtype=np.float64).reshape(-1)
        mode_count = min(int(self.active_mode_count), int(self.response_matrix.shape[1]))
        response = np.asarray(self.response_matrix[:, :mode_count], dtype=np.float64)
        if response.size == 0:
            return np.zeros(int(self.response_matrix.shape[1]), dtype=np.float64)
        if self.tikhonov_lambda <= 0.0:
            solved = np.linalg.pinv(response, rcond=SHWFS_RCOND) @ measurements
        else:
            mode_weights = np.linspace(1.0, 2.5, mode_count, dtype=np.float64)
            gram = response.T @ response
            rhs = response.T @ measurements
            regularizer = float(self.tikhonov_lambda) * np.diag(mode_weights * mode_weights)
            solved = np.linalg.solve(gram + regularizer, rhs)
        coeffs = np.zeros(int(self.response_matrix.shape[1]), dtype=np.float64)
        coeffs[:mode_count] = np.asarray(solved, dtype=np.float64).reshape(-1)[:mode_count]
        return coeffs
...
def _build_active_lenslet_cells(
    ...
    *,
    ...
    return _build_active_lenslet_layout(
        pupil_x,
        pupil_y,
        valid_mask,
...
def _build_active_lenslet_layout(
    pupil_x: np.ndarray,
    pupil_y: np.ndarray,
    valid_mask: np.ndarray,
    *,
    ...
    active_cell_indices = tuple(np.asarray(item[3], dtype=np.int64) for item in selected)
    active_cell_linear_indices = tuple(int(item[2]) for item in selected)
    return active_cell_indices, active_cell_linear_indices
...
def _build_shwfs_measurement_model(
    ...
    *,
    ...
    mechanical_dof_names: tuple[str, ...] = ("dx_mm", "dy_mm", "dz_mm"),
    tilt_step_deg: float = 0.0,
    build_mechanical_response: bool = True,
    ...
    active_cell_indices, active_cell_linear_indices = _build_active_lenslet_layout(
    ...
    shwfs = ShwfsMeasurementModel(
        ...
        active_cell_linear_indices=active_cell_linear_indices,
    ...
    reference_coeffs = shwfs._solve_modal_coeffs(reference_slopes)
    ...
    if build_mechanical_response:
        ...
        if shwfs.mechanical_response_matrix.size > 0:
            ...
            shwfs.mechanical_response_pinv = np.empty((0, 0), dtype=np.float64)
    else:
        shwfs.mechanical_response_matrix = np.empty((reference_slopes.size, 0), dtype=np.float64)
        shwfs.mechanical_response_pinv = np.empty((0, 0), dtype=np.float64)
    return shwfs
