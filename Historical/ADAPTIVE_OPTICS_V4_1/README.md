# Adaptive Optics V4.1

This folder is the `global-TT-first` successor to `ADAPTIVE_OPTICS_V4`.
...
V4.1 keeps the V4 hidden-plant policy, but changes the tip/tilt path from a single flat TT block into a two-step TT search.
...
8. Run the main translation blocks first.
9. For strongly angular or tip/tilt-dominant cases, apply a light `XY/Z` regularization before the TT stages so the controller is less eager to fake tip/tilt repair with translation-only compensation.
10. Run a `global TT` block first, using only two common-mode variables: `global_tilt_x` and `global_tilt_y`.
11. Follow with a `local TT refine` block that allows per-anchor TT residuals while lightly penalizing TT variance across anchors.
12. If a block stalls, keep its current best state and continue into the next block instead of aborting the whole repair.
...
Use these files for the V4.1 workflow:

- [Launch_Adaptive_Optics_V4_1.cmd](</c:/Users/arthu/OneDrive/文档/_UCSB/ECE/Adaptive Optics/ADAPTIVE_OPTICS_V4_1/Launch_Adaptive_Optics_V4_1.cmd>)
- [ao_v4_1_launcher.py](</c:/Users/arthu/OneDrive/文档/_UCSB/ECE/Adaptive Optics/ADAPTIVE_OPTICS_V4_1/ao_v4_1_launcher.py>)
- [ao_v4_1_worker.py](</c:/Users/arthu/OneDrive/文档/_UCSB/ECE/Adaptive Optics/ADAPTIVE_OPTICS_V4_1/ao_v4_1_worker.py>)
- [ao_v4_1_backend.py](</c:/Users/arthu/OneDrive/文档/_UCSB/ECE/Adaptive Optics/ADAPTIVE_OPTICS_V4_1/ao_v4_1_backend.py>)
