# Adaptive Optics V10.1

`ADAPTIVE_OPTICS_V10_1` is the restored **8-mode mainline**.

This branch keeps the sensor-compatible controller work, but the current benchmark target is the 8-mode line that proved most stable in the profile comparison:

- `minimal` is the best 8-mode profile in `artifacts/v10_profile_sensor_trial/results.csv`
- `realistic` remains the recommended 8-bit working profile for comparison
- `hard` remains the stress-test profile

## Best 8-Mode Result

Current best 8-mode reference:

- [artifacts/v10_profile_sensor_trial/results.csv](</c:/Users/arthu/OneDrive/文档/_UCSB/ECE/Adaptive%20Optics/ADAPTIVE_OPTICS_V10_1/artifacts/v10_profile_sensor_trial/results.csv>)

Best profile call path:

```powershell
conda run -n ao311 python .\ADAPTIVE_OPTICS_V10_1\ao_v10_1_launcher.py
```

If you only want the best 8-mode snapshot, start from the `minimal` profile in that table. The branch is still compatible with the older V9/V10 launchers, but the V10.1 launcher is the cleanest entry point.

## Multi-Sensor Benchmark Notes

`ADAPTIVE_OPTICS_V10_1` still carries the multi-sensor compatibility layer:
