...
TIPTILT_TEST_LIMIT_ARCMIN = float(os.environ.get("AO_V4_TIPTILT_TEST_LIMIT_ARCMIN", "5.0"))
BLOCK_STAGE_MAXITER = int(os.environ.get("AO_V4_BLOCK_STAGE_MAXITER", "2"))
...
TT_DOMINANT_ANGULAR_RATIO = float(os.environ.get("AO_V4_TT_DOMINANT_ANGULAR_RATIO", "2.0"))
TT_DOMINANT_XYZ_BIAS_WEIGHT = float(os.environ.get("AO_V4_TT_DOMINANT_XYZ_BIAS_WEIGHT", "0.05"))
...
def _zero_centered_search_bounds(actuator_ids: list[int], *, search_box_radius_mm: float) -> list[tuple[float, float]]:
    ...
    for _surface_id in actuator_ids:
        bounds.extend(
            [
                (-float(search_box_radius_mm), float(search_box_radius_mm)),
                (-float(search_box_radius_mm), float(search_box_radius_mm)),
                (-float(search_box_radius_mm), float(search_box_radius_mm)),
            ]
        )
...
def run_repair(config: RunConfig, log: LogCallback | None = None) -> dict[str, Any]:
    ...
    bounds = _zero_centered_search_bounds(ACTUATOR_IDS, search_box_radius_mm=search_box_radius_mm)
    ...
    try:
            ...
            try:
                ...
                if ENABLE_TIPTILT and block_indices["tt"].size > 0:
                    current_vector = optimize_block(
                        ...
                        current_vector,
                        block_indices["tt"],
                        stage_tt_limit_deg=min(float(TIPTILT_LOCAL_LIMIT_DEG), float(TIPTILT_TEST_LIMIT_ARCMIN / 60.0)),
                    )
                final_primary_order = _choose_primary_block_order(inspect_signature(current_vector))
                current_vector = optimize_block(
                    f"stage4_{final_primary_order[0]}_polish",
                    current_vector,
        ...
        summary = {
            "status": "complete",
            "algorithm": "adaptive_optics_v4_residual_guided_block_search",
            "configuration": {
                "wavelength_nm": float(bench.WAVELENGTH_NM),
                "object_space_mm": float(bench.OBJECT_SPACE_MM),
                "reference_focus_mm": float(bench.REFERENCE_FOCUS_MM),
