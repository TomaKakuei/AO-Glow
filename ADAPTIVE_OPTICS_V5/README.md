...
`ADAPTIVE_OPTICS_V5` builds on `V4.1` and keeps the same hidden-plant closed-loop truth boundary, but replaces the old slope-level SHWFS noise hack with a `13 x 13` noisy sensor forward model injected in the electron domain.
...
- `realistic`
  Recommended default. Moderate gain variation, background offset, shot noise, and low CMOS read noise.
- `enhanced`
