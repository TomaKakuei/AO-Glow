...
LENSLET_COUNT = _env_int("AO_V3_SHWFS_LENSLET_COUNT", 8)
SHWFS_MAXITER = 8
SHWFS_MAXFUN = 120
SHWFS_RCOND = 1.0e-3
SHWFS_SLOPE_LIMIT = _env_int("AO_V3_SHWFS_SLOPE_LIMIT", 256)
MECHANICAL_RESPONSE_STEP_MM = 5.0e-3
MECHANICAL_RESPONSE_STEP_SCHEDULE_MM = (5.0e-3,)
MECHANICAL_RESPONSE_RCOND = 1.0e-6
SHWFS_NOISE_MIN_FRACTION = max(0.0, _env_float("AO_V3_SHWFS_NOISE_MIN_FRACTION", 0.0))
SHWFS_NOISE_MAX_FRACTION = max(
    SHWFS_NOISE_MIN_FRACTION,
    _env_float("AO_V3_SHWFS_NOISE_MAX_FRACTION", 0.0),
)
SHWFS_NOISE_SEED = _env_int("AO_V3_SHWFS_NOISE_SEED", 20260430)
...
class ShwfsMeasurementModel:
    ...
    noise_min_fraction: float = SHWFS_NOISE_MIN_FRACTION
    noise_max_fraction: float = SHWFS_NOISE_MAX_FRACTION
    ...
    def measure_slopes(self, phase_waves: np.ndarray) -> np.ndarray:
        ...
        for flat_indices in self.active_cell_indices:
            ...
            slope_xy = np.asarray(coeffs[:2], dtype=np.float64)
            if self.noise_max_fraction > 0.0:
                noise_mag = float(self.rng.uniform(self.noise_min_fraction, self.noise_max_fraction))
                noise_sign = -1.0 if float(self.rng.random()) < 0.5 else 1.0
                slope_xy = slope_xy * (1.0 + noise_sign * noise_mag)
    ...
    def measure_model_slopes(self, optical_model: RayOpticsPhysicsEngine) -> np.ndarray:
        return self.measure_slopes(self._current_phase_waves(optical_model))
