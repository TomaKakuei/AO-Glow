...
TIPTILT_TEST_LIMIT_ARCMIN = float(os.environ.get("AO_V6_TIPTILT_TEST_LIMIT_ARCMIN", os.environ.get("AO_V5_TIPTILT_TEST_LIMIT_ARCMIN", os.environ.get("AO_V4_1_TIPTILT_TEST_LIMIT_ARCMIN", "6.0"))))
BLOCK_STAGE_MAXITER = int(os.environ.get("AO_V6_BLOCK_STAGE_MAXITER", os.environ.get("AO_V5_BLOCK_STAGE_MAXITER", os.environ.get("AO_V4_BLOCK_STAGE_MAXITER", "2"))))
...
TT_DOMINANT_ANGULAR_RATIO = float(os.environ.get("AO_V6_TT_DOMINANT_ANGULAR_RATIO", os.environ.get("AO_V5_TT_DOMINANT_ANGULAR_RATIO", os.environ.get("AO_V4_TT_DOMINANT_ANGULAR_RATIO", "2.0"))))
TT_DOMINANT_XYZ_BIAS_WEIGHT = float(os.environ.get("AO_V6_TT_DOMINANT_XYZ_BIAS_WEIGHT", os.environ.get("AO_V5_TT_DOMINANT_XYZ_BIAS_WEIGHT", os.environ.get("AO_V4_1_TT_DOMINANT_XYZ_BIAS_WEIGHT", "0.03"))))
TT_LOCAL_VARIANCE_WEIGHT = float(os.environ.get("AO_V6_TT_LOCAL_VARIANCE_WEIGHT", os.environ.get("AO_V5_TT_LOCAL_VARIANCE_WEIGHT", os.environ.get("AO_V4_1_TT_LOCAL_VARIANCE_WEIGHT", "0.02"))))
DEFAULT_SHWFS_NOISE_PROFILE = os.environ.get("AO_V6_SHWFS_NOISE_PROFILE", os.environ.get("AO_V5_SHWFS_NOISE_PROFILE", "realistic")).strip().lower() or "realistic"
DEFAULT_SHWFS_LENSLET_COUNT = 13
DEFAULT_SHWFS_SLOPE_LIMIT = 256
DEFAULT_SHWFS_FORWARD_AVERAGES = int(os.environ.get("AO_V6_SHWFS_FORWARD_AVERAGES", os.environ.get("AO_V5_SHWFS_FORWARD_AVERAGES", "1")))
SEARCH_BOX_XY_OVERRIDE_MM = float(os.environ.get("AO_V6_SEARCH_BOX_XY_MM", os.environ.get("AO_V5_SEARCH_BOX_XY_MM", "0.0")) or "0.0")
SEARCH_BOX_Z_OVERRIDE_MM = float(os.environ.get("AO_V6_SEARCH_BOX_Z_MM", os.environ.get("AO_V5_SEARCH_BOX_Z_MM", "0.0")) or "0.0")
...
def _configure_v6_shwfs_runtime(config: RunConfig) -> str:
...
def generate_before_calibration(config: RunConfig, log: LogCallback | None = None) -> dict[str, Any]:
    ...
    noise_profile_name = _configure_v6_shwfs_runtime(config)
    logger(
        "Configured V6 SHWFS runtime: "
...
def run_repair(config: RunConfig, log: LogCallback | None = None) -> dict[str, Any]:
    ...
    def _with_forward_averages(forward_averages: int, fn: Callable[[], Any]) -> Any:
        previous = int(shwfs.forward_average_count)
        shwfs.forward_average_count = max(1, int(forward_averages))
        try:
            return fn()
        finally:
            shwfs.forward_average_count = previous
    ...
    def optimize_block(
        ...
        *,
        ...
        local_log(
            ...
            f"forward_averages={int(stage_forward_averages)} "
        ...
        try:
            stage_result = _with_forward_averages(
                stage_forward_averages,
                lambda: minimize(
                    reduced_objective,
                    x0=x0_reduced,
                    method="L-BFGS-B",
        ...
        stage_records.append(
            {
                ...
                "forward_averages": int(stage_forward_averages),
    ...
    tt_stage_forward_averages = max(1, int(config.shwfs_forward_averages))
    non_tt_stage_forward_averages = 1
    try:
        ...
        with warnings.catch_warnings(record=True) as warning_records:
            ...
            try:
                ...
                current_vector = optimize_block(
                    ...
                    stage_forward_averages=non_tt_stage_forward_averages,
                )
                current_vector = optimize_block(
                ...
                if ENABLE_TIPTILT and block_indices["tt"].size > 0:
                    ...
                    current_vector = optimize_global_tt_block(
                        ...
                        stage_forward_averages=tt_stage_forward_averages,
                    )
                    current_vector = optimize_block(
                        "stage4_tt_local",
                        current_vector,
        ...
        summary = {
            "status": "complete",
            "algorithm": "adaptive_optics_v6_robust_noisy_shwfs_block_search",
            "configuration": {
                "wavelength_nm": float(bench.WAVELENGTH_NM),
                "object_space_mm": float(bench.OBJECT_SPACE_MM),
                "reference_focus_mm": float(bench.REFERENCE_FOCUS_MM),
                ...
                "shwfs_active_mode_count": int(shwfs.active_mode_count),
                "shwfs_tikhonov_lambda": float(shwfs.tikhonov_lambda),
                "shwfs_weighted_centroid_power": float(shwfs.weighted_centroid_power),
                "shwfs_window_radius_px": int(shwfs.window_radius_px),
                "shwfs_dynamic_threshold_sigma": float(shwfs.dynamic_threshold_sigma),
                "shwfs_gaussian_sigma_px": float(shwfs.gaussian_sigma_px),
                "shwfs_enable_median_filter": bool(shwfs.enable_median_filter),
                "shwfs_enable_gaussian_filter": bool(shwfs.enable_gaussian_filter),
